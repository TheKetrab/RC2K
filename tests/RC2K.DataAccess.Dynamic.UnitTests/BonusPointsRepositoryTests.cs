...
public class BonusPointsRepositoryTests
{
    private Mock<Container> _container;
    private BonusPointsRepository _sut;
    ...
    public void SetUp()
    {
        (var db, _container, var mapper, var envProvider) = 
            ContainerMockHelper.SetUpMocks<BonusPointsMapper>("BonusPoints");

        _sut = new BonusPointsRepository(db.Object, mapper.Object, envProvider.Object);

        _container.
            Setup(x => x.ReadItemAsync<BonusPointsModel>(
                It.IsAny<string>(),
    ...
    public async Task Container_WhenUsePartitionKey_ShouldEqualEntityName()
    {
        ...
        //Assert
        _container
            .Verify(x => x.ReadItemAsync<BonusPointsModel>(
                It.Is<string>(y => y == id.ToString()),
                It.Is<PartitionKey>(y => y.ToString() == "[\"BonusPoints\"]"),
                It.IsAny<ItemRequestOptions>(),
                It.IsAny<CancellationToken>()));
